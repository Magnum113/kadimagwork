...
## Spacing audit — 01.09.2026

### Исправлено

- Легенда графика отделена от оси самостоятельным интервалом `16px`; элементы легенды могут переноситься строкой с отдельным вертикальным gap.
- Нижний padding карточки графика увеличен до `22px`, поэтому легенда не прижата ни к графику, ни к краю карточки.
- Подзаголовок целей интернет-магазина отделён от графика на `32px`, а от связанных карточек — на `16px`: межгрупповой интервал вдвое больше внутригруппового.

### Проверка

- Desktop `1440 × 900`: passed.
- Mobile `390 × 844`: passed.
- Вертикальный ритм заголовков, карточек, таблиц и footer: passed; дополнительных изменений не требуется.

### Final result

passed

## Hero background update — 01.09.2026

### Исправлено

- Фон заменён на широкую Apple `largetall`-версию `6016 × 1384`.
- Изображение растянуто на всю площадь hero без отдельной заливки слева, поэтому вертикальный стык удалён.
- На desktop яблоко смещено в правую визуальную зону и отделено от текстового блока свободным пространством.
- На mobile задан отдельный масштаб и позиция: яблоко показывается целиком, а текст остаётся читаемым на затемнённой части фона.

### Проверка

- Desktop `1440 × 900`: passed, стык отсутствует.
- Mobile `390 × 844`: passed, кадрирование и читаемость сохранены.
- Browser console errors / warnings: не найдены.
